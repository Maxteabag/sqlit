...
const remote = /^https?:\/\//.test(reportArg);
const report = remote ? reportArg : path.resolve(reportArg), output = path.resolve(outArg);
const reportUrl = remote ? report : pathToFileURL(report).href;
const reportOrigin = remote ? new URL(report).origin : null;
...
(async () => {
  ...
  page.on('request', request => { if (/^https?:/.test(request.url()) && (!remote || new URL(request.url()).origin !== reportOrigin)) externalRequests.push(request.url()); });
  ...
  if (remote) {
    for (const href of localLinks) {
      const resource = await page.request.get(new URL(href, report).href);
      assert.ok(resource.ok(), `Missing live resource: ${href} (${resource.status()})`);
    }
  } else {
  ...
  if (remote) {
    const resource = await page.request.get(new URL('diagnostics/completion-baseline.trace.json', report).href);
    assert.ok(resource.ok());
    await page.setInputFiles('#trace-file', { name: 'completion-baseline.trace.json', mimeType: 'application/json', buffer: await resource.body() });
  } else {
  ...
  const result = { reportUrl, desktop: [1440, 1000], mobile: [390, 844], figures: 11, localLinks: localLinks.length,
