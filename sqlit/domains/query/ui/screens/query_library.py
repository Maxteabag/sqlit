...
class QueryLibraryScreen(ModalScreen[SavedQueryEntry | None]):
    ...
    BINDINGS = [
        ...
        Binding("slash", "open_filter", "Search"),
    ]
    ...
    #query-library-filter { background: $surface; }
    ...
    def compose(self) -> ComposeResult:
        with Dialog(
            ...
            yield FilterInput(id="query-library-filter")
            if self.entries:
                yield OptionList(
    ...
    def on_mount(self) -> None:
        ...
        self.query_one("#query-library-filter", FilterInput).hide()
    ...
    def _matches(self, entry: SavedQueryEntry, query: str) -> bool:
        ...
        return query_lower in entry.relative_path.lower() or query_lower in entry.query.lower()

    def _apply_filter(self) -> None:
        self._filter_fuzzy = self._filter_text.startswith("~")
        query = self._filter_text[1:] if self._filter_fuzzy else self._filter_text
        self._display_entries = [
            entry for entry in self.entries if self._matches(entry, query)
        ]
        filter_input = self.query_one("#query-library-filter", FilterInput)
        filter_input.set_filter(
            self._filter_text,
            len(self._display_entries),
            len(self.entries),
        )
        self._rebuild_list()
    ...
    def action_open_filter(self) -> None:
        ...
        self._filter_text = ""
        self.query_one("#query-library-filter", FilterInput).show()
        self._apply_filter()
    ...
    def _close_filter(self) -> None:
        ...
        self._filter_text = ""
        self._filter_fuzzy = False
        self._display_entries = list(self.entries)
        self.query_one("#query-library-filter", FilterInput).hide()
        self._rebuild_list()
    ...
    def on_key(self, event: Any) -> None:
        if not self._filter_active:
            if event.key == "q":
                ...
                event.stop()
            elif event.key in {"j", "k"} and self.entries:
                option_list = self.query_one("#query-library-list", OptionList)
                if event.key == "j":
    ...
    def on_paste(self, event: events.Paste) -> None:
        if not self._filter_active or not event.text:
            return
